...
public:
  ...
  const uint8_t ACTION_RETURN_VALUE_ARRAY[32] = {
      195, 166, 19, 140, 80,  97,  207, 41,  19,  16,  136,
      124, 11,  92, 113, 252, 175, 254, 171, 144, 213, 222,
      181, 13,  59, 158, 104, 124, 234, 212, 80,  113 };

  const checksum256 ACTION_RETURN_VALUE_DIGEST =
      checksum256( ACTION_RETURN_VALUE_ARRAY );
  ...
  static uint32_t reverse_bytes( uint32_t input ) {
    uint32_t output = ( input >> 24 & 0xff ) | ( input >> 8 & 0xff00 ) |
  ...
  TABLE blockheader {
    ...
    checksum256 digest() const {
      ...
      checksum256         hFull =
          sha256( serializedFull.data(), serializedFull.size() );

      return hFull;
    }
    uint32_t block_num() const { return get_block_num_from_id( previous ) + 1; }
    checksum256 block_id() const {
      return compute_block_id( digest(), block_num() );
    }

    EOSLIB_SERIALIZE(
        blockheader,
        (timestamp)(producer)(confirmed)(previous)(transaction_mroot)(action_mroot)(schedule_version)(new_producers)( header_extensions ) )
  };
  ...
  TABLE sblockheader {
    ...
    EOSLIB_SERIALIZE(
        sblockheader,
        (header)(producer_signatures)(previous_bmroot)( bmproofpath ) )
  };
  ...
  TABLE actionproof {
    ...
    EOSLIB_SERIALIZE( actionproof,
                      (action)(receipt)(returnvalue)( amproofpath ) )
  };
  ...
  TABLE chain {
    ...
    EOSLIB_SERIALIZE( chain,
                      (name)(chain_id)(return_value_activated)( enabled ) )
  };
  ...
  TABLE chainschedule {
    ...
    EOSLIB_SERIALIZE(
        chainschedule,
        (version)(producer_schedule_v1)(producer_schedule_v2)(hash)(first_block)(last_block)( expiry ) )
  };
  ...
  TABLE lastproof {
    ...
    EOSLIB_SERIALIZE( lastproof,
                      (id)(block_height)(block_merkle_root)( expiry ) )
  };
  ...
  typedef eosio::multi_index<
      "chains"_n,
      chain,
      indexed_by< "chainid"_n,
                  const_mem_fun< chain, checksum256, &chain::by_chain_id > > >
      chainstable;
  typedef eosio::multi_index<
      "schedules"_n,
      chainschedule,
      indexed_by< "expiry"_n,
                  const_mem_fun< chainschedule,
                                 uint64_t,
                                 &chainschedule::by_expiry > > >
      chainschedulestable;
  typedef eosio::multi_index<
      "lastproofs"_n,
      lastproof,
      indexed_by<
          "height"_n,
          const_mem_fun< lastproof, uint64_t, &lastproof::by_block_height > >,
      indexed_by<
          "merkleroot"_n,
          const_mem_fun< lastproof, checksum256, &lastproof::by_merkle_root > >,
      indexed_by<
          "expiry"_n,
          const_mem_fun< lastproof, uint64_t, &lastproof::by_expiry > > >
      proofstable;
  ...
  void checkactionproof( checksum256 chain_id,
  ...
  checksum256 get_next_schedule_hash( name     chain_name,
                                      uint32_t schedule_version );
