...
void check_signatures( name                     producer,
  ...
  checksum256 digest_to_sign =
      hash_pair( std::make_pair( header_bmroot, producer_schedule_hash ) );
  ...
  if ( block.header.header_extensions.size() > 0 ) {
    for ( const auto &ext : block.header.header_extensions ) {
      if ( ext.first == 1 ) {
        ...
        producer_schedule_hash =
            sha256( new_producer_schedule_hash_packed.data(),
                    new_producer_schedule_hash_packed.size() );
      }
    }
  }
...
checksum256 generate_action_digest( const bridge::r_action    &act,
...

  {
    ...
    std::copy( serialized_data.cbegin(),
               serialized_data.cend(),
               result.begin() );
    std::copy( serialized_output.cbegin(),
               serialized_output.cend(),
               result.begin() + action_input_size );
...
void bridge::gc_schedules( name chain, int count ) {
  ...
  int distance = std::distance( _schedulestable.begin(),
                                _schedulestable.end() ); //todo : optimize
...
void bridge::check_proven_root( name chain, checksum256 root ) {
  ...
  check( _proofstable.begin() != _proofstable.end(),
         "no root has been proved yet" );
...
ACTION bridge::inita( name              chain_name,
                      ...
                      producer_schedule initial_schedule ) {
...
ACTION bridge::initb( name               chain_name,
                      checksum256        chain_id,
                      uint32_t           return_value_activated,
                      bridge::schedulev2 initial_schedule ) {
...
void bridge::checkactionproof( checksum256 chain_id,
  ...
  check( actionDigest == actionproof.receipt.act_digest,
         "digest of action doesn't match the digest in action_receipt" );

  std::vector< char > serializedReceipt = pack( actionproof.receipt );
  checksum256         receiptDigest =
      sha256( serializedReceipt.data(), serializedReceipt.size() );
  ...
  if ( actionproof.amproofpath.size() == 1 &&
    ...
    check( blockheader.action_mroot == receiptDigest,
           "invalid action merkle proof path" );
  } else
    check( proof_of_inclusion( actionproof.amproofpath,
                               receiptDigest,
...
void bridge::checkblockproof( heavyproof blockproof ) {
  //___PHASE 1___: Verify block header
  auto cid_index = _chainstable.get_index< "chainid"_n >();
  auto chain_itr = cid_index.find( blockproof.chain_id );

  // CODE-REVIEW: check if chain is enabled after validating if hte chain is supported, index coulnd't exist
  ...
  if ( block_num > sched_itr->last_block && !schedule_hash_updated ) {
    ...
    if ( new_schedule_format )
      producer_schedule_hash =
          get_next_schedule_hash( chain_itr->name,
                                  sched_itr->producer_schedule_v2.version + 1 );
    else
      producer_schedule_hash =
          get_next_schedule_hash( chain_itr->name,
                                  sched_itr->producer_schedule_v1.version + 1 );
  ...
  checksum256 id =
      compute_block_id( header_digest,
                        blockproof.blocktoprove.block.header.block_num() );
  ...
  if ( new_schedule_format )
    bm_root = check_block_header( blockproof.blocktoprove.block,
                                  active,
                                  node_count,
  ...
  if ( new_schedule_format )
    threshold_for_finality = producer_schedule_v2.producers.size() * 2 / 3;
  else
    threshold_for_finality = producer_schedule_v1.producers.size() * 2 / 3;

  check( blockproof.bftproof.size() == threshold_for_finality * 2,
         "invalid number of bft proofs" );
  ...
  for ( int i = 0; i < blockproof.bftproof.size(); i++ ) {
    std::vector< checksum256 > bmproofpath =
        map_hashes( blockproof.hashes, blockproof.bftproof[i].bmproofpath );
    ...
    check( proof_of_inclusion( bmproofpath,
                               id,
                               blockproof.bftproof[i].previous_bmroot ),
           "invalid block merkle proof path" );

    header_digest =
        blockproof.bftproof[i]
            .header.digest(); // compute current block's header digest
    id = compute_block_id(
        header_digest,
        blockproof.bftproof[i]
            .header
            .block_num() ); // compute current BFT proof block id for next verification
    uint32_t bft_schedule_version =
        blockproof.bftproof[i].header.schedule_version;

    check(
        bft_schedule_version == schedule_version ||
            bft_schedule_version == schedule_version + 1,
        "invalid schedule version in BFT proof" ); //can only have at most one schedule change over the time period required to prove an action
    ...
    if ( new_schedule_format )
      check_signatures( blockproof.bftproof[i].header.producer,
                        blockproof.bftproof[i].producer_signatures,
                        header_digest,
                        blockproof.bftproof[i].previous_bmroot,
                        producer_schedule_v2,
                        producer_schedule_hash );
      ...
      check_signatures( blockproof.bftproof[i].header.producer,
                        blockproof.bftproof[i].producer_signatures[0],
                        header_digest,
                        blockproof.bftproof[i].previous_bmroot,
                        producer_schedule_v1,
                        producer_schedule_hash );
  ...
  check( round1_bft_producers.size() == threshold_for_finality,
         "not enough BFT proofs to prove finality" );
  check( round2_bft_producers.size() == threshold_for_finality,
         "not enough BFT proofs to prove finality" );
  ...
  if ( blockproof.blocktoprove.block.header.new_producers.has_value() ) {
    ...
    check( sched_itr != _schedulestable.end(),
           "must prove missing schedules in correct sequence" );
  ...
  if ( blockproof.blocktoprove.block.header.header_extensions.size() > 0 ) {
    for ( const auto &ext :
          blockproof.blocktoprove.block.header.header_extensions ) {
      if ( ext.first == 1 ) {
        auto new_schedule_hash_packed = ext.second;

        // unpack new schedule
        schedulev2 new_producer_schedule =
            unpack< bridge::schedulev2 >( new_schedule_hash_packed );
        auto schedule_hash = sha256( new_schedule_hash_packed.data(),
                                     new_schedule_hash_packed.size() );

        // set end_block for previous schedule
        auto sched_itr =
            _schedulestable.find( new_producer_schedule.version - 1 );
        check( sched_itr != _schedulestable.end(),
               "must prove missing schedules in correct sequence" );
...
void bridge::_checkproofb( heavyproof blockproof, actionproof actionproof ) {
  ...
  checkblockproof( blockproof );
  checkactionproof( blockproof.chain_id,
                    blockproof.blocktoprove.block.header,
                    actionproof );
...
void bridge::_checkproofc( lightproof blockproof, actionproof actionproof ) {
  ...
  checksum256 id =
      compute_block_id( headerDigest, blockproof.header.block_num() );

  check( proof_of_inclusion( blockproof.bmproofpath, id, blockproof.root ),
         "invalid block merkle proof" );
